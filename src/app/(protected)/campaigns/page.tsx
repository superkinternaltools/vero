...
import { listCampaignStatuses } from "@/modules/org/queries";
import { CampaignsClient } from "@/modules/campaigns/components/campaigns-client";

export default async function CampaignsPage() {
  await requireAccess("campaigns");
  const [campaigns, statuses] = await Promise.all([listCampaigns(), listCampaignStatuses()]);

  return <CampaignsClient campaigns={campaigns} statuses={statuses} />;
}

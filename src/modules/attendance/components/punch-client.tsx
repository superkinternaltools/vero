...
import type { PunchContext, PunchAssignment } from "../types";
...
function ShiftCard({
  ...
  a: PunchAssignment;
  busy: boolean;
  onTake: (assignmentId: string, kind: Kind) => void;
}) {
  const doneKinds = new Set(a.punches.map((p) => p.kind));
  const punchTime = (kind: string) => {
    const p = a.punches.find((x) => x.kind === kind);
  ...
  function CaptureButton({ kind, label }: { kind: Kind; label: string }) {
    return (
      <button
        type="button"
        disabled={busy}
        onClick={() => onTake(a.assignmentId, kind)}
        className={cn(
...
export function PunchClient({ ctx }: { ctx: PunchContext }) {
  ...
  const [capturing, setCapturing] = useState<{ assignmentId: string; kind: Kind } | null>(null);

  useEffect(() => {
    if (ctx.assignments.length > 0 && "geolocation" in navigator) {
  ...
  async function onCapture(blob: Blob) {
    ...
    setCapturing(null);
    if (!pick) return;
    const a = ctx.assignments.find((x) => x.assignmentId === pick.assignmentId);
    if (!a) return;
    setUploading(true);
    setError(null);
    const supabase = createClient();
    const path = `${a.workDate}/${crypto.randomUUID()}.jpg`;
  ...
  const dateLabel = new Date(ctx.today + "T00:00:00Z").toLocaleDateString("en-IN", {
      ...
      {ctx.assignments.length === 0 ? (
        ...
        ctx.assignments.map((a) => (
          <ShiftCard
            ...
            a={a}
            busy={busy}
            onTake={(assignmentId, kind) => setCapturing({ assignmentId, kind })}
          />
        ))
      )}

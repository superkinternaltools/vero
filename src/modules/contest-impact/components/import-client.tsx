...
function parseInventoryRows(text: string): { rows: InventorySourceRow[]; error: string | null } {
  ...
  const required = ["month", "week", "storeName", "campaignName", "skuName"];
  const missing = required.filter((k) => !idx.has(k));
  if (missing.length) return { rows: [], error: `Missing column(s): ${missing.join(", ")}` };
  const get = (r: string[], key: string) => (idx.has(key) ? (r[idx.get(key)!] ?? "") : "");
  const rows = table
    ...
    <div>
      ...
      <div className="mt-6 space-y-4">
        <SourceImportCard<CampaignSourceRow>
          ...
          headers={["month", "week", "store_name", "campaign_name", "status"]}
          stores={stores}
        ...
        <SourceImportCard<InventorySourceRow>
          ...
          description="One row per store, per campaign, per week, per SKU — target vs. actual stock, store and warehouse."
          headers={["month", "week", "store_name", "campaign_name", "sku_name", "target_store_stock", "in_store_stock", "target_warehouse_stock", "in_warehouse_stock"]}
          stores={stores}

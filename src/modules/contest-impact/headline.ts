...
- gmv, sellThrough, doh, inStoreValue: average per store per week, by group. sellThrough is GMV ÷ inStoreValue — a turnover ratio (e.g. 0.14 means 14% of that week's shelf stock sold). doh (days of hand) is 7 × inStoreValue ÷ GMV — the exact reciprocal of sellThrough, in days: how long the current stock would last at that week's sales pace. They're the same underlying fact in two units — prefer whichever reads more naturally for the specific point you're making ("carrying 50 days of stock vs. 12" often lands better than a turnover ratio). Always write sellThrough with an "x" suffix ("0.14x") and doh with a "d" suffix ("50d") — never a bare percentage for either.
...
- comparisons: precomputed magnitudes — use these, don't compute your own from the raw values (you will get the arithmetic wrong):
  ...
  - approvedVsPoorSellThroughPct / approvedVsControlSellThroughPct: relative % difference between their sell-through ratios — a relative %, not a point gap.
...
- sellThrough is a ratio: always give the actual values ("0.135x vs 0.152x") alongside the relative-% framing — never state a bare "%" gap on a ratio with no raw figures next to it, since the reader can't tell if that's a relative percent or a percentage-point gap.
- Any gap between two growth-% or percentage figures (like poorVsControlGrowthPtDiff) is stated in PERCENTAGE POINTS ("pp"), never plain "%" — a point gap and a relative percent change are different things and must never be conflated or left ambiguous.
...
BAD (do not write like this): "Approved stores' sales fell nearly 10% short of last month, lagging well behind poor-execution stores' GMV growth." / "The approved group's GMV decreased by 10.1%, while the poor group's GMV was only 1.1% lower. Additionally, approved stores exhibited a sell-through ratio that was 4.6% weaker than their poor counterparts, illustrating that poor execution led to stronger performance in sales despite their lower conversion rate."
Why it's bad: it never mentions the approved-vs-control verdict (the one comparison that actually answers "did the contest work"); it frames poor's GMV as a positive ("stronger performance," "despite lower conversion rate") instead of a red flag; and "4.6% weaker" has no raw sellThrough values next to it, so the reader can't tell if that's a big gap or a rounding error.
GOOD (write like this): "Approved execution is still outselling control by ₹X/week (Y% above baseline), even though poor-execution stores posted similar raw GMV." / "Approved stores converted 0.135x of shelf stock versus poor stores' 0.152x — about 11% higher turnover — so poor's comparable GMV came from carrying more stock, not better execution, and doesn't change the verdict that approval-gated execution is what's driving incremental sales. [Second sentence: whatever other caveat is most relevant, e.g. year-on-year data thinness, or poor barely beating control's own pace.]"
...
2. Is every sellThrough percentage paired with the actual "x" ratio values it was computed from?

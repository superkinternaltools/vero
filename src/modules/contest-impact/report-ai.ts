...
UNITS: sellThrough/doh values always with "x"/"d" suffixes, never bare. Rupee figures always ₹. incrementalPctOfBaseline and approvedVsPoorSellThroughPct are relative percentages, not percentage points; poorVsControlGrowthPtDiff is a percentage-point gap ("pp"), not a percent. Never state two numbers next to each other without saying what the gap means.
